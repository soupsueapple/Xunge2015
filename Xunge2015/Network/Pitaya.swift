...
public class PitayaManager {
    ...
    let userAgent: String = {
        if let info : NSDictionary = NSBundle.mainBundle().infoDictionary {
            
            guard let executable = info.objectForKey(kCFBundleIdentifierKey) as AnyObject?,
                  let bundle = info.objectForKey(kCFBundleIdentifierKey) as AnyObject?,
                  let version = info.objectForKey(kCFBundleIdentifierKey) as AnyObject?,
                let os: AnyObject = NSProcessInfo.processInfo().operatingSystemVersionString as AnyObject? else{
                    return "Pitaya"
            }
    ...
    func buildParams(parameters: [String: AnyObject]) -> String {
        ...
        let keys = Array(parameters.keys).sort(){ $0 < $1}
